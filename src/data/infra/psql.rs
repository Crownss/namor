...
lazy_static! {
    static ref THECLIENT: AsyncOnce<Res<Pool<PostgresConnectionManager<NoTls>>>> =
...
pub async fn new_connection() -> Res<Pool<PostgresConnectionManager<NoTls>>> {
    ...
    let the_pool = bb8::Pool::builder()
        .max_size(config.psql.max_pool as u32)
        .build(conn_manager.clone())
        .await
        .expect("cannot create connection for pool postgres");
...
pub async fn check_connection() -> Res<()> {
    info!("Checking on postgresql connection...");
